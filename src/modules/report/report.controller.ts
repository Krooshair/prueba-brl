...
class ReportController extends ReportService {
  public async listReportGlobal(
    req: Request,
    res: Response
  ): Promise<Response> {
    try {
      const { year } = req.query;
      if (!isNumeric(Number(year)))
      ...
      return res.status(500).json({
        message:
          'Ocurrio un error inesperado al intentar listar el reporte global',
      });
    }
  }

  public async listReportByAdmin(
    req: Request,
    res: Response
  ): Promise<Response> {
    try {
      ...
      return res.status(500).json({
        message:
          'Ocurrio un error inesperado al intentar listar el reporte global por personal',
      });
    }
  }

  public async listReportByClient(
    req: Request,
    res: Response
  ): Promise<Response> {
    try {
      ...
      return res.status(500).json({
        message:
          'Ocurrio un error inesperado al intentar listar el reporte global por cliente',
      });
    }
  }

  public async listReportByBusiness(
    req: Request,
    res: Response
  ): Promise<Response> {
    try {
      ...
      return res.status(500).json({
        message:
          'Ocurrio un error inesperado al intentar listar el reporte global por negocio',
      });
    }
  }

  public async downloadReportForMonth(
    req: Request,
    res: Response
  ): Promise<Response> {
    try {
  ...
  public async downloadReportGlobal(
    req: Request,
    res: Response
  ): Promise<Response> {
    try {
      ...
      const adminId = req.query.adminId
        ? parseInt(req.query.adminId as string, 10)
        : undefined;
      const businessId = req.query.businessId
        ? parseInt(req.query.businessId as string, 10)
        : undefined;

      const response = await this.getReportForDownload(
        Number(year),
        month,
        clientId,
        adminId,
  ...
  public async downloadReportByBusiness(
    req: Request,
    res: Response
  ): Promise<Response> {
    try {
      ...
      return res.status(500).json({
        message:
          'Ocurrio un error inesperado al intentar descargar el reporte global por negocio',
      });
    }
  }
}
