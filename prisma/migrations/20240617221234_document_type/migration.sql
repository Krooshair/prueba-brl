/*
  ...
  - You are about to alter the column `documentType` on the `admin` table. The data in that column could be lost. The data in that column will be cast from `VarChar(255)` to `Enum(EnumId(4))`.
  ...
  - You are about to alter the column `documentType` on the `client` table. The data in that column could be lost. The data in that column will be cast from `VarChar(255)` to `Enum(EnumId(4))`.
...
*/
-- AlterTable
ALTER TABLE `admin` MODIFY `documentType` ENUM('DNI', 'RUC', 'Carnet_Extranjeria') NOT NULL,
...
-- AlterTable
ALTER TABLE `client` MODIFY `documentType` ENUM('DNI', 'RUC', 'Carnet_Extranjeria') NOT NULL,
    MODIFY `createdAt` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
    MODIFY `updatedAt` DATETIME NOT NULL;
