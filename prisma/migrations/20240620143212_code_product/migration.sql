/*
  ...
  - A unique constraint covering the columns `[codeProduct]` on the table `Product` will be added. If there are existing duplicate values, this will fail.
  - Added the required column `codeProduct` to the `Product` table without a default value. This is not possible if the table is not empty.
...
-- AlterTable
ALTER TABLE `product` ADD COLUMN `codeProduct` VARCHAR(255) NOT NULL,
...
-- CreateIndex
CREATE UNIQUE INDEX `Product_codeProduct_key` ON `Product`(`codeProduct`);
